...
function detectCameraBrand(text) {
  ...
  if (t.includes("sony") || t.includes("alpha") || t.includes("ilce-")) return "sony";
  if (t.includes("canon") || t.includes("eos")) return "canon";
...
function looksLikeSonyA6400MainTitle(titleText) {
  ...
  if (!hasExactSonyA6400Signal(t)) return false;
  if (hasWrongSonyAlphaModel(t)) return false;
...
function isHardAccessoryListing(text, item, family = "") {
  ...
  if (isAccessoryCategory(item) && !looksLikeMainCameraTitle(titleText, family)) {
    return true;
  }
...
function getMatchDebug(item, queryContext) {
  ...
  if (isHardAccessoryListing(text, item, queryContext.family || "")) {
    return { matched: false, reason: "accessory_listing", title: titleText };
  }
  if (isClearlyNonCamera(item, text, queryContext.family || "")) {
    return { matched: false, reason: "non_camera_listing", title: titleText };
  }
